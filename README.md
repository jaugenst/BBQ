...
This workflow that can be found in the "bacterial_burden_volume.ipynb" is summarized as follow : Step 1 and 2 are identical to the first workflow.
3. Bacterial channel extraction. This time the channel is smoothed and saved as a z-stack with the extension "_bact.tif".
4. Voronoi segmentation on the nuclei channel image and cell ROIs generation. The ROIs from cellpose output are generated and used to perform voronoi segmentation. A new set of ROIs is then generated based on the result and the RoiSet is saved.
5. the "bact" image is opened, and the Voronoi ROIs used to collect the total area of the bacterial signal per cell in every slice. the bacterial signal was thresholded first and transformed in a binary image. 
6. The table generated by the measure is first saved. From the results, the total bacterial volume is calculated following this equation : 
\begin{equation}
Volume =\sum_{i=1}^\n \A_ixZ_i
\end{equation}
where A is the total area measured in the slice _i_, and Z is the interval in um between 2 slices.
